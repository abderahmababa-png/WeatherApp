...
if st.button("📊 توليد وتحليل التدوينة الجوية", use_container_width=True):
    with st.spinner(f"جاري معالجة خرائط ونماذج {selected_city}..."):
        try:
            ...
            # تحديد روابط خوادم الأرصاد الجوية بناءً على المدى الزمني
            if h > 120:
            ...
            if weather_res.status_code != 200 or (h > 120 and "hourly" not in weather_res.json()):
                ...
                h = min(h, 168)  # حد التوقعات الاحتياطية المتوفرة
                
            prayer_res = requests.get(prayer_url, timeout=10)
            
            if weather_res.status_code == 200:
                response_json = weather_res.json()
                data = response_json.get("hourly", {})
                
                if not data or "temperature_2m" not in data:
                    st.error("⚠️ فشل في معالجة مصفوفة البيانات المستلمة. يرجى إعادة المحاولة لاحقاً.")
                else:
...
{moisture_influence} وبالتالي تبقى فرص الهطول الفعلي منعدمة عند **0.0 ملم** مع تراجع احتمالية الأمطار لـ **{prob}%**."""
                ...
                st.error(f"⚠️ عذراً، خادم النماذج الجوية غير مستقر حالياً. (رمز الخطأ: {weather_res.status_code})")
