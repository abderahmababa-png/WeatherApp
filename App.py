...
strings = {
    "العربية": {
        ...
        "current_status": "🌤️ الحالة اللحظية للطقس اليوم",
        "temp_label": "الحرارة الحالية",
        "sky_label": "السماء",
        "wind_label": "الرياح",
        "max_temp": "العظمى المتوقعة",
    ...
    "English": {
        ...
        "current_status": "🌤️ Real-time Weather Status",
        "temp_label": "Temp",
        "sky_label": "Sky",
        "wind_label": "Wind",
        "max_temp": "Max Temp",
...
# إدارة عرض شعار التطبيق الدائري بشكل مصغر جداً لتوفير المساحة
...
# 3. النطاق الجغرافي للبلديات والمقاطعات (في سطر واحد مدمج)
locations_map = {
...
# 4. معالجة وتوليد البيانات بضغطة زر واحدة
    ...
    with st.spinner(".."):
        try:
            ...
            if data:
                ...
                prob = max([int(x) if x is not None else 0 for x in data.get("precipitation_probability", [])[:actual_h]])
                wind_speeds = [float(x) if x is not None else 0.0 for x in data.get("wind_speed_10m", [])[:actual_h]]
                max_wind = max(wind_speeds)
                max_t = max(temps)
                
                # البيانات الفورية الحالية لحفظ العمود الممتد
                current_temp_c = temps[0]
                current_wind = wind_speeds[0]
                sky_status = "ممطر 🌧️" if precip_list[0] > 0.1 else ("غائم ☁️" if prob > 40 else "مشمس ☀️")
                if lang != "العربية":
                    sky_status = sky_status.replace("ممطر", "Rainy").replace("غائم", "Cloudy").replace("مشمس", "Sunny")

                # 🌟 عرض كل المؤشرات (اللحظية والتحليلية) في سطر عرضي واحد مدمج ومختصر المساحة
                ...
                # 📅 اختصار المدى الأسبوعي داخل ميزة الأكورديون (الطي القابل للتوسيع) لصفر مساحة افتراضية
                with st.expander(txt["weekly_title"], expanded=False):
                    ...
                    for idx in range(0, actual_h, 24):
                        ...
                        if day_chunk_t:
                            ...
                            day_rain = sum(precip_list[idx:idx+24])
                            day_wind = max(wind_speeds[idx:idx+24])
                ...
                # قسم المدونة والنصائح (في كتل نصية صغيرة الحجم وضيقة)
                st.markdown(f"<h5 style='margin:4px 0; {is_rtl}'>{txt['expert_blog']}</h5>", unsafe_allow_html=True)
                blog = f"استقرار مؤشر {selected_city} ذروة {format_temp(max_t)}، احتمالية أمطار {prob}% بتراكم {precip:.1f}ملم."
                st.info(blog)
                
                # مواقيت الصلاة والواتساب مدموجة تماماً بالأسفل
