...
# تجميل الواجهة وإخفاء شريط أدوات التطوير (GitHub & Fork)
st.markdown(
    """
    <style>
...
# القواميس والنصوص البرمجية لواجهة نصرة
strings = {
    "ar": {
...
with st.sidebar:
    if os.path.exists(LOGO_FILE):
        ...
        st.markdown(
            f"<h2 style='text-align: center; color: #1E3A8A;'>{txt['title']}</h2>",
            unsafe_allow_html=True
        )
        
    st.markdown(
        f"<h3 style='text-align: center; color: #1E3A8A;'>{txt['prayer_title']}</h3>",
        unsafe_allow_html=True
    )
    
    st.markdown(
        f"<p style='text-align: center; color: gray; font-size: 13px;'>{txt['prayer_sub']}</p>",
        unsafe_allow_html=True
    )
...
# واجهة الصفحة الرئيسية للمنصة
st.title(txt["title"])

st.markdown(
    f"<h4 style='color: gray;'>{txt['subtitle']} في مقاطعة <span style='color: #1E88E5; font-weight: bold;'>{selected_city}</span></h4>",
    unsafe_allow_html=True
)
...
if weather_json:
    ...
    with col1:
        st.metric(
            label=txt["temp"],
            value=format_temp(current['temperature'])
        )
    ...
    with col3:
        st.metric(
            label=txt["wind"],
            value=f"{current['windspeed']} {txt['wind_speed_unit']}"
        )
    ...
    with col4:
        ...
        st.metric(
            label=txt["humidity"],
            value=f"{current_humidity}{txt['humidity_unit']}"
        )
    ...
    st.markdown(
        f"<p style='color: gray; font-size: 15px;'>{txt['radar_desc']}</p>",
        unsafe_allow_html=True
    )
...
st.markdown(
    f"<p style='text-align: center; color: #1E3A8A; font-weight: bold; font-size: 14px;'>{txt['footer_text']}</p>",
    unsafe_allow_html=True
)
